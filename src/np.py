...
class NeuralProcess(nn.Module):
    ...
    def forward(self,
        ...
        z_prior_dist = self.latent_encoder(x_context, y_context)

        if self.training:
            assert y_target is not None, "y_target must be provided during training"
            z_post_dist = self.latent_encoder(x_target, y_target) # assumes that x_target includes the x_context points
